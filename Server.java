...
public class Server {
    ...
    public static void broadcastPositions() {
        ...
        for (Player p : playerStates.values()) {
            sb.append(p.id()).append(",").append(p.x()).append(",").append(p.y())
              .append(",").append(p.isChaser() ? "1" : "0")
              .append(",").append(p.isEliminated() ? "1" : "0").append(";");
        }
    ...
    public static void checkForTags() {
        ...
        for (Player p : playerStates.values()) {
            if (!p.id().equals(chaserId) && !p.isEliminated()) {
                if (Math.abs(p.x() - chaser.x()) < 40 && Math.abs(p.y() - chaser.y()) < 40) {
                    p.setEliminated(true);
                    System.out.println("[TAG] " + p.id() + " has been eliminated!");
                }
            }
        }

        long remaining = playerStates.values().stream().filter(p -> !p.isEliminated() && !p.isChaser()).count();
        if (remaining <= 0) {
            System.out.println("[GAME] Chaser wins!");
